...
files <- c("SMKA12", "SMKE19", "SMKI19", "SIAI11", "SMKX46", "SMKM46", "SESX16", "SESM16")
names(files) <- c("control", "exp", "imp", "traders", "disp", "arr", "dispest", "arrest")
...
tradedata = dbConnect(pg, user="datascience7", password="",
...
dbSendQuery(tradedata, "alter table dispatches alter column smk_nett_mass type bigint")

dbWriteTable(tradedata,'arrivals', arrivals, row.names=FALSE)
dbSendQuery(tradedata, "delete from arrivals")
dbSendQuery(tradedata, "alter table arrivals alter column smk_nett_mass type bigint")

dbWriteTable(tradedata,'exports', exports, row.names=FALSE)
dbSendQuery(tradedata, "delete from exports")
dbSendQuery(tradedata, "alter table exports alter column value type bigint")
dbSendQuery(tradedata, "alter table exports alter column quantity_1 type bigint")
dbSendQuery(tradedata, "alter table exports alter column quantity_2 type bigint")

dbWriteTable(tradedata, 'imports', imports, row.names=FALSE)
dbSendQuery(tradedata, "delete from imports")
dbSendQuery(tradedata, "alter table imports alter column value type bigint")
dbSendQuery(tradedata, "alter table imports alter column quantity_1 type bigint")
dbSendQuery(tradedata, "alter table imports alter column quantity_2 type bigint")

#dbWriteTable(tradedata, 'control', control, row.names=FALSE)
#dbSendQuery(tradedata, "delete from control")

yrs <- as.character(sprintf("%02d", c(9:15)))
#yrs <- as.character(sprintf("%02d", 15))
